...
function normalizeMetadata(metadata) {
  ...
  const nestedName = thread.name?.text;
  const nestedDescription = thread.description?.text;
  const nestedInvite = thread.invite;

  return {
    id: metadata.id,
    name: metadata.name ?? nestedName ?? 'Unknown Channel',
    description: metadata.description ?? nestedDescription ?? '',
    invite: metadata.invite ?? nestedInvite ?? '',
...
async function buildQuotedContent(message) {
  const quoted = normalizeMessageContent(getQuotedMessage(message));
  if (!quoted) return null;

  if (quoted.imageMessage) {
    return {
      image: await readMediaBuffer(quoted.imageMessage, 'image'),
      ...(quoted.imageMessage.caption ? { caption: quoted.imageMessage.caption } : {}),
    };
  }

  if (quoted.videoMessage) {
    return {
      video: await readMediaBuffer(quoted.videoMessage, 'video'),
      ...(quoted.videoMessage.caption ? { caption: quoted.videoMessage.caption } : {}),
    };
  }

  if (quoted.audioMessage) {
    return {
      audio: await readMediaBuffer(quoted.audioMessage, 'audio'),
  ...
  if (quoted.documentMessage) {
    return {
      document: await readMediaBuffer(quoted.documentMessage, 'document'),
      mimetype: quoted.documentMessage.mimetype ?? 'application/octet-stream',
      fileName: quoted.documentMessage.fileName ?? 'file',
      ...(quoted.documentMessage.caption ? { caption: quoted.documentMessage.caption } : {}),
    };
  }

  if (quoted.stickerMessage) {
    return {
      sticker: await readMediaBuffer(quoted.stickerMessage, 'sticker'),
    };
  }

  if (quoted.conversation || quoted.extendedTextMessage?.text) {
    return { text: getQuotedText(message) };
  }
...
export function createNewsletterService() {
  async function getCurrentChannel(context) {
    if (!isNewsletterJid(context.chatId)) {
      throw new Error('Gunakan command ini di dalam Channel atau kirim link/JID channel.');
    }
    return resolveReference(context.socket, context.chatId);
  }

  async function resolve(context, reference) {
    return resolveReference(context.socket, reference);
  }
  ...
  async function setSaved(context, metadata) {
    if (!metadata?.id) throw new Error('Metadata channel tidak valid.');
    context.repositories.settings.set(
      CHANNEL_SCOPE,
      context.senderJid,
      CHANNEL_KEY,
  ...
  async function assertCanPost(context, metadata) {
    ...
    if (role && !['ADMIN', 'OWNER'].includes(role)) {
      throw new Error(`Akun ini bukan admin channel. Role: ${role}`);
    }
  }
  ...
  async function upload(context, text = '') {
    ...
    const quotedContent = await buildQuotedContent(context.message);
    const bodyText = String(text).trim();
    const content = quotedContent ?? (bodyText ? { text: bodyText } : null);
    if (!content) {
      throw new Error(`Balas pesan yang ingin di-upload atau tulis teks setelah ${context.config.prefix}upch.`);
    }

    if (typeof context.socket.sendMessage !== 'function') {
      throw new Error('Baileys sendMessage tidak tersedia.');
    }
