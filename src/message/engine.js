...
export function createMessageEngine({ config, logger, identity, registry, repositories, economy, providers }) {
  ...
  function rememberNewsletterMessage(message) {
    ...
    newsletterMessages.set(`${chatId}:${messageId}`, {
      serverId: String(serverId), participant: message.key?.participant ?? null,
      fromMe: Boolean(message.key?.fromMe), messageId,
  ...
  function rememberGroupMessage(message) {
    ...
    cache.set(messageId, {
      message, senderJid: message.key?.participant ?? chatId,
      pushName: getPushName(message, message.key?.participant ?? chatId),
      createdAt: Date.now(), expiresAt: Date.now() + settings.ttl,
    });
  ...
  async function handleMessage(socket, message) {
    ...
    const context = createMessageContext({
      socket, message, command, registry, identity, config, parsed, providers, repositories, economy,
      resolveNewsletterMessage, resolveGroupMessage,
    });
    ...
    try {
      ...
      if (slowReactionShown) await safeReact(context, '❌');
      try { await context.reply('Terjadi kesalahan saat menjalankan command. Coba lagi nanti.'); } catch {}
  ...
  async function handleDeletedMessages(socket, keys) {
    for (const key of keys ?? []) {
      ...
      try {
        await socket.sendMessage(destination, { text: info, mentions: sender && sender.includes('@') ? [sender] : [] });
  ...
  async function handleParticipantsUpdate(socket, event) {
    ...
    for (const participant of event.participants) {
      ...
      try { await socket.sendMessage(event.id, { text, mentions: [jid] }); }
      catch (error) { logger.error(`Error ${action === 'add' ? 'welcome' : 'left'} = ${error?.message ?? String(error)}`); }
    }
  }
  ...
  function attach(socket) {
    ...
    socket.ev.on('connection.update', async ({ connection }) => {
      if (connection === 'open' && config.autoOnline) try { await socket.sendPresenceUpdate('available'); } catch (error) { logger.debug('Presence update failed', { error: error?.message ?? String(error) }); }
    });
  }
