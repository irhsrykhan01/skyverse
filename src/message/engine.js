...
export function createMessageEngine({ config, logger, identity, registry, repositories }) {
  ...
  async function handleMessage(socket, message) {
    ...
    if (config.autoRead) {
      ...
      catch (error) { logger.warn('Failed to mark message as read', { error: error?.message ?? String(error) }); }
    }
    ...
    try {
      ...
      logger.error('Command execution failed', { command: command.name, sender: context.senderJid, chat: chatId, error: error?.message ?? String(error) });
      try { await context.reply('Terjadi kesalahan saat menjalankan command. Coba lagi nanti.'); }
      catch (replyError) { logger.warn('Failed to send command error response', { error: replyError?.message ?? String(replyError) }); }
    }
  }
  ...
  function attach(socket) {
    ...
    socket.ev.on('connection.update', async ({ connection }) => {
      if (connection === 'open' && config.autoOnline) {
        try { await socket.sendPresenceUpdate('available'); logger.info('WhatsApp presence set to online'); }
        catch (error) { logger.warn('Failed to set online presence', { error: error?.message ?? String(error) }); }
      }
    });
  }
